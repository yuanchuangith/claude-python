...
- `src/claude_agent_sdk/actiondesign_gateway/review_protocol.py`
...
Review 响应格式保持结构化 JSON，不返回前端工具调用：
...
`/review` endpoint 强制模型只输出 JSON。若模型返回 `[TOOL_CALL]` / `[TOL_CALL]` 或任何前端工具调用，后端直接返回 `success: false`、`pass: false`、`code: REVIEW_TOOL_CALL_NOT_ALLOWED`。若模型没有返回单个合法 JSON object，返回 `REVIEW_RESPONSE_INVALID_JSON`。
...
MiMo review 入口 `call_mimo_review()` 不执行 backend tool loop，也不返回 ActionDesign 工具调用。它通过专用 review system prompt 要求上游只返回 CodeReview JSON，并由 `review_protocol.py` 做 JSON 解析和工具调用拦截。
...
Claude Code review 入口 `call_claude_code_review()` 复用 Claude Code 只读项目能力，但最终同样只返回 CodeReview JSON；任何文本或 native frontend tool 调用都会按 review 协议拦截为失败。
...
## MiMo 和 Claude Code knowledge prompt

MiMo 和 Claude Code 的 backend-only prompt 约束：

- 如果不确定 ActionDesign 元件的 `inputParams`、`outputParams`、事件、方法、示例、约束或必填参数结构，必须先调用 `knowledge.search`，不要猜测后直接写画布。
- search 结果不够时再调用 `knowledge.read`。
- 包含 `knowledge.search/read` 的回复不能同轮混合 `create_action`、`create_node`、`insert_node`、`delete_node` 等前端写工具；必须等待 backend tool result 后下一轮继续。
- knowledge 适合查询通用元件规则、组件约定、方法/事件契约、示例和限制。
- 当前画布、已有动作、页面组件实例和实际组件方法优先使用前端只读工具：`inspect_action`、`get_page_actions`、`get_element_detail`、`get_component_methods`。
...
Claude Code provider 仍保留自己的 `Read/Grep/Glob/LS` 项目文件只读能力，不开放写类内部工具给前端。
...
- 路径：默认写入 `logs/actiondesign-agent/yyyyMMdd_HHmmss_{conversationId}.jsonl`；文件名前缀使用首次创建该对话日志文件时的中国时间 UTC+8、24 小时制，例如 `20260610_183045_conv_xxx.jsonl`；可用 `ACTIONDESIGN_FULL_CONVERSATION_LOG_ROOT` 覆盖根目录。
...
- `/api/actiondesign-agent/tool-result` 收到前端工具结果后，额外追加 `tool_result` 到同一个 `yyyyMMdd_HHmmss_{conversationId}.jsonl` 文件；如果该 conversationId 的时间前缀日志文件已存在，会复用已有文件而不是按回传时间新建文件。
...
- provider 专用 `/review` endpoint 返回 CodeReview JSON，且不包含 `tool_calls`。
...
- MiMo 和 Claude Code backend prompt 明确 read-before-write：不确定元件参数、方法、事件、示例或约束时先调用 `knowledge.search/read`。
- MiMo 和 Claude Code backend prompt 明确 `knowledge.search/read` 不能同轮混用前端写工具。
...
- MiMo review 能解析合法 CodeReview JSON，并在 system prompt 中强制 `Return only valid JSON`。
- MiMo review 遇到 `[TOOL_CALL]` / `[TOL_CALL]` 前端工具调用时返回 `REVIEW_TOOL_CALL_NOT_ALLOWED`。
- Claude Code review 能解析合法 CodeReview JSON。
...
```powershell
$env:PYTHONPATH='src'
python -m pytest tests/test_actiondesign_gateway_mimo_backend_loop.py tests/test_actiondesign_gateway_tool_protocol.py -q
```
...
## 2026-06-10 知识库主动查询 prompt 强化

- 本次没有新增自动 preflight，也没有做模型生成工具调用后的滞后查询；仍由模型主动输出 `[BACKEND_TOOL_CALL] knowledge.search/read`，后端执行并回灌。
- MiMo 和 Claude Code backend prompt 增加 read-before-write 规则：不确定 ActionDesign 元件参数、组件方法、事件、示例或约束时，必须先查询知识库或前端只读工具，再生成写画布工具调用。
- prompt 明确包含 `knowledge.search/read` 的同一轮回复不能混合 `create_action`、`create_node`、`insert_node`、`delete_node` 等前端写工具，避免边查边写。
- prompt 明确知识库用于通用元件规则、组件约定、方法/事件契约、示例和限制；当前画布、已有动作、页面组件实例和实际组件方法仍优先使用 `inspect_action`、`get_page_actions`、`get_element_detail`、`get_component_methods`。
- 新增最小 prompt 回归测试，覆盖 MiMo 和 Claude Code 的 read-before-write 规则，防止后续提示词退化。

## 2026-06-10 完整会话日志文件名中国时间前缀

- 完整会话 JSONL 文件名从 `{conversationId}.jsonl` 改为 `yyyyMMdd_HHmmss_{conversationId}.jsonl`，前缀使用文件首次创建时的中国时间 UTC+8、24 小时制。
- `conversationId` 本身、响应头 `X-Conversation-Id` / `X-Run-Id`、JSONL 事件结构和普通脱敏调试日志路径保持不变。
- `append_conversation_event()` 写入前会先查找 `????????_??????_{conversationId}.jsonl`，找到则追加到已有文件；找不到才用当前中国时间创建新文件，确保 `/tool-result` 回传继续落到同一对话文件。

## 2026-06-10 v2 Reviewer endpoint

- 新增 provider 专用 review 路由：`POST /api/actiondesign-agent/mimo/review` 和 `POST /api/actiondesign-agent/claude-code/review`。
- 新增 CodeReview 模型：`CodeReviewRequest`、`CodeReviewResponse`、`CodeReviewIssue`。响应使用 `pass/summary/issues/success/error/code/duration_ms/usage/raw`，不包含 `tool_calls`。
- 新增 `review_protocol.py`，集中处理 reviewer system prompt、JSON 解析、invalid JSON 失败和工具调用拦截。
- MiMo review 使用 Anthropic-compatible Messages API 单轮调用，不执行 backend tool loop；Claude Code review 复用 Claude Code 只读能力，但最终也必须返回 JSON。
- 如果 reviewer 返回 `[TOOL_CALL]` / `[TOL_CALL]` 或 native frontend tool call，后端返回 `REVIEW_TOOL_CALL_NOT_ALLOWED`，用于防止 Reviewer endpoint 写画布。
